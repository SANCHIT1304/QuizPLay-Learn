...
import { Play, BookOpen, Clock, Users, Star, Zap } from 'lucide-react';

const SkillTrack = () => {
  const { track } = useParams();
  const navigate = useNavigate();
  
  const trackTitle = track.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()).replace(/And/g, '&');
  const [playlists, setPlaylists] = useState([]);
  
      useEffect(() => {
        fetch(`http://localhost:3000/api/playlists/skills/subject/${trackTitle}`)
          .then(res => res.json())
          .then(setPlaylists);
      }, [trackTitle]);

  // Mock playlist data for skill tracks
  // const playlists = [
  //   {
  //     id: 1,
  //     title: trackTitle + ' Basics',
  ...
  //   {
  //     id: 2,
  //     title: trackTitle + ' Intermediate Projects',
  ...
  //   {
  //     id: 3,
  //     title: trackTitle + ' Advanced Techniques',
  ...
  //   {
  //     id: 4,
  //     title: trackTitle + ' Real-World Applications',
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Build practical skills in {trackTitle.toLowerCase()} with hands-on projects and expert guidance
            </p>
          </motion.div>
